#!/usr/bin/env bash
#
# Compile cb-spmv (src/) and run it serially over every matrix in data/.
# Matrices are referenced by a relative path but converted to an absolute
# path at run time, because the program requires an absolute path.
#
set -euo pipefail
